...
class TeapotTest {
	...
	@Disabled
	void testTeapot1() {
		System.out.println("RUNNING: Teapot 1 - No Optimizations");
		// useSS: false, useASS: false, threads: 0 (ריצה רגילה)
		runAndMeasureTeapot(false, false, 0, "teapot1_no_optimizations");
	}

	/**
	 * 10 Teapot tests with your optimizations (formerly CBR -> now Multithreading)
	 */
	@Test
	@Disabled
	void testTeapot2() {
		System.out.println("RUNNING: Teapot 2 - Threads & Regular Super Sampling");
		// useSS: true, useASS: false, threads: -2 (ריבוי תהליכונים)
		runAndMeasureTeapot(true, false, -2, "teapot2_threads_and_SS");
	}

	/**
	 * 100 Teapot tests with your optimizations (formerly BVH -> now Adaptive Super
	 * Sampling)
	 */
	@Test
	void testTeapot3() {
		System.out.println("RUNNING: Teapot 3 - Threads & Adaptive Super Sampling");
		// useSS: false, useASS: true, threads: -2 (ריבוי תהליכונים ואדפטיבי)
	 ...
	 * builder with all the data for the test
	 */
	Camera.Builder prepareTeapot() {
		...
		scene._geometries.add(bubbles);

		// שים לב: בטסט החדש השתמשת ב setKQ, אם מתקבלת שגיאה כאן על setKq, שנה לאות
		// גדולה
		scene.lights.add(new PointLight(new Color(500, 500, 500), new Point(100, 0, -100)).setKQ(0.000001));

		return Camera.getBuilder() //
				.setResolution(1000, 1000) //
				.setRayTracer(scene, RayTracerType.SIMPLE) // הדרך הנכונה לפרויקט שלך
				.setLocation(new Point(0, 0, -1000))
				// תיקון הוקטורים: המצלמה במיקום Z=-1000 וצריכה להסתכל על (0,0,0), לכן כיוון
				// ההסתכלות הוא (0,0,1)
				.setDirection(new Vector(0, 0, 1), new Vector(0, 1, 0)).setVpDistance(1000).setVpSize(200, 200);
	}

	/**
	 * מתודת עזר שמגדירה את ה-Builder של הקומקום, מפעילה את הרינדור, שומרת תמונה,
	 * מודדת זמן במדויק ומדפיסה אחוזי התקדמות.
	 */
	private void runAndMeasureTeapot(boolean useSS, boolean useASS, int threadsCount, String imageName) {
		// בניית הבסיס של הקומקום והפעלת מד האחוזים בטרמינל
		Camera.Builder builder = prepareTeapot().setMultithreading(threadsCount).setDebugPrint(2); // <<< מד האחוזים

		// 1. הגדרת מנגנון ה-Super Sampling הרגיל
		if (useSS) {
			builder.setSuperSampling(true, 9, 9, 1.0, SamplingGrid.AreaShape.SQUARE,
					SamplingGrid.SamplingPattern.REGULAR);
		} else {
			// במצב כבוי (או כאשר ASS פעיל ומנהל את ה-Sampling בעצמו)
			builder.setSuperSampling(false, 1, 1, 1.0, SamplingGrid.AreaShape.SQUARE,
					SamplingGrid.SamplingPattern.REGULAR);
		}

		// 2. הגדרת מנגנון ה-Adaptive Super Sampling האדפטיבי
		if (useASS) {
		...
		// ביצוע הציור ושמירת התמונה
		builder.build().renderImage().printGrid(50, new Color(YELLOW)) // שימוש בצבע הצהוב הקיים בטסט
				.writeToImage(imageName);

		long endTime = System.currentTimeMillis();
		long duration = endTime - startTime;

		System.out.println(">> Test [" + imageName + "] completed in: " + duration + " ms");
		System.out.println("========================================================");
	}
	...
	private static final Material material = new Material().setKD(0.5).setKS(0.5).setNShininess(60);
