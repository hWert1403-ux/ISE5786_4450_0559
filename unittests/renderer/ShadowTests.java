...
class ShadowTests {
	...
	private final Camera.Builder _cameraBuilder = Camera.getBuilder().setLocation(new Point(0, 0, 1000))
			.setVpDistance(1000).setVpSize(200, 200).setDirection(Point.ZERO, Vector.AXIS_Y)
			// .setDirection(new Point(-8, -78, 0), Vector.AXIS_Y) // debug
			.setResolution(600, 600).setRayTracer(_scene, RayTracerType.SIMPLE);

	/** The sphere in the tests */
	private final Intersectable _sphere = new Sphere(new Point(0, 0, -200), 60D).setEmission(new Color(BLUE))
			.setMaterial(new Material().setKD(0.5).setKS(0.5).setNShininess(99));
	/** The material of the triangles in the tests */
	private final Material _trMaterial = new Material().setKD(0.5).setKS(0.5).setNShininess(30);
	...
	private void sphereTriangleHelper(String pictName, Triangle triangle, Point spotLocation) {
		_scene._geometries.add(_sphere, triangle.setEmission(new Color(BLUE)).setMaterial(_trMaterial));
		_scene.lights //
	...
	void testSphereTriangleMove1() {
		// הזזת קודקודי המשולש קצת למעלה וקצת ימינה (בסביבות 8-10 יחידות בצירי X ו-Y)
	...
	void testSphereTriangleMove2() {
		// הזזת קודקודי המשולש עוד יותר למעלה וימינה (בסביבות 18-20 יחידות מהמיקום
		// המקורי)
	...
	void testSphereTriangleSpot1() {
		// קירוב מקור האור אל הסצנה (הקטנת ערך ה-Z והזזתו מעט בצירי X ו-Y)
	...
	void testSphereTriangleSpot2() {
		// קירוב משמעותי של מקור האור קרוב מאוד אל המשולש והספרה
	...
	private void setTrianglesSphereScene() {
		Material triangleMaterial = new Material().setKD(0.2).setKS(0.4).setNShininess(111);
		_scene._geometries //
				.add( //
						...
						new Sphere(new Point(0, 0, -11), 30D) //
								...
								.setMaterial(new Material().setKD(0.5).setKS(0.4).setNShininess(111)) //
				);
